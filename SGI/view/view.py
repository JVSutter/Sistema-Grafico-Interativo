...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def define_values(self):
        ...
        zoom = 50
        self.zoomSlider.setValue(zoom) # MUDAR PARA O ZOOM DEFINIDO NO MODEL
    ...
    def connect_buttons(self):
        ...
        self.createPoint.clicked.connect(self.create_point)
        self.createLine.clicked.connect(self.create_line)
        self.createWireframe.clicked.connect(self.create_wireframe)
    ...
    def add_object(self, points, name):
        """Adiciona um objeto a lista de objetos da aplicacao"""
        
        self.controller.handle_point_input(points, name) # Adiciona o objeto ao controller
    ...
    def create_point(self):
        ...
        if name != None:
            ...
            self.add_log(f"Point {name} created: {points}")
    ...
    def create_line(self):
        ...
        if name != None:
            self.add_object(points, name)
            self.add_log(f"Line {name} created: {points}")
    ...
    def create_wireframe(self):
        ...
        if name != None:
            self.add_object(points, name)
            self.add_log(f"Wireframe {name} created: {points}")
