...
class ObjectDialog(QtWidgets.QDialog):
    ...
    def create_object(self):
        ...
        is_filled = self.fillCheckBox.isChecked() if self.fillCheckBox.isEnabled() else False
...
class EdgeDialog(QtWidgets.QDialog):
    ...
    def __init__(self, points: list[tuple[float, float, float]]):
        ...
        self.points = points
        self.edges = []  # Lista para armazenar as arestas como tuplas de índices (idx1, idx2)
        ...
        # Conecta botoes
        self.addEdgeButton.clicked.connect(lambda: self.add_edge(self.selectPoint1.currentIndex(), self.selectPoint2.currentIndex()))
    ...
    def _update_select_boxes(self):
        ...
        self.addEdgeButton.setEnabled(idx1 != idx2 and tuple(sorted((idx1, idx2))) not in self.edges)
    ...
    def add_edge(self, idx1: int, idx2: int):
        ...
        if idx1 == idx2:
            self.show_error_message("The initial and final points of the edge cannot be the same.")
            return
        ...
        if edge in self.edges:
            self.show_error_message(f"Edge between Point {edge[0]} and Point {edge[1]} already exists.")
            return
